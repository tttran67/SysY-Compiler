...
void Function::output() const
{
    ...
    else
    {
        fprintf(yyout, "define %s %s(", retType->toStr().c_str(),
                sym_ptr->toStr().c_str());
        for (long unsigned int i = 0; i < paramsType.size()-1; i++) {
            // if (i)
            //     fprintf(yyout, ", ");
            fprintf(yyout, "%s %s, ", paramsType[i]->toStr().c_str(), (func_params[i])->toStr().c_str());
        }
        fprintf(yyout, "%s %s) {\n", paramsType[paramsType.size()-1]->toStr().c_str(), (func_params[func_params.size()-1])->toStr().c_str());
    }
...
void Function::genMachineCode(AsmBuilder* builder) //lab7
{
